<template>
  <div class="m-3">
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-2xl font-bold">Custom Tour Requests</h2>
      <!-- No Create button, as these are typically user-submitted requests -->
    </div>

    <n-card>
      <n-data-table
        :columns="columns"
        :data="props.customTours.data"
...
const columns = computed(() => [
  ...
  { title: 'Email', key: 'email', width: 220, sorter: true, sortOrder: currentSortBy.value === 'email' ? currentSortOrder.value : false },
  { title: 'Country', key: 'country.name', width: 150, sorter: true, render: (row) => row.country?.name || 'N/A', sortOrder: currentSortBy.value === 'country.name' ? currentSortOrder.value : false },
  { title: 'Note', key: 'note', ellipsis: { tooltip: true } },
  { title: 'IP Address', key: 'ip', width: 130, sorter: true, sortOrder: currentSortBy.value === 'ip' ? currentSortOrder.value : false },
  { 
    title: 'Requested At', 
    key: 'created_at', 
    width: 180, 
    ...
    render(row) {
      return h(NSpace, null, {
        default: () => [
          h(
            Link,
            { href: route('admin.custom_tours.show', row.id) }, // Assuming a show route
          ...
          h(
            ...
            },
            {
              ...
              default: () => 'Are you sure you want to delete this request?',
            }
          ),
        ],
      });
...
const paginationProps = computed(() => ({
  page: props.customTours.current_page,
  pageSize: props.customTours.per_page,
  itemCount: props.customTours.total,
  pageSlot: 7,
  showSizePicker: true,
  pageSizes: [10, 20, 50, 100].map(size => ({ label: String(size), value: size })),
  prefix: ({ itemCount }) => `${itemCount} total requests`,
}));
...
const fetchData = (page, pageSize, sortBy, sortOrder, activeFilters) => {
  ...
  loading.value = true;
  Inertia.get(route('admin.custom_tours.index'), queryParams, {
    preserveState: true,
    replace: true,
    preserveScroll: true,
...
const handleDelete = (id) => {
  Inertia.delete(route('admin.custom_tours.destroy', id), {
    preserveScroll: true,
    onStart: () => { loading.value = true; },
    onSuccess: () => { alert('Custom tour request deleted successfully!'); },
    onError: () => { alert('Failed to delete custom tour request.'); },
    onFinish: () => { loading.value = false; },
  });
};
