    ...
    <div class="container-fluid d-flex justify-content-between">
        ...
        <div>
          ...
          <n-drawer v-model:show="show" :width="502">
            <n-drawer-content title="Create category" closable>
              <div class="container">
                <n-input-number class="my-1" v-model:value="form.category_id" clearable placeholder="category_id"/>
                ...
                <div class="btn btn-success" @click="form.post(route('admin.categories.store'), {onSuccess:()=>{form.reset()}, preserveState:false})">Submit</div>
              </div>
            </n-drawer-content>
          </n-drawer>
        </div>
  ...
  render(row) {
    return h(
      'div',
      {
        ...
        onClick: () => Inertia.put(route('admin.categories.update', row.id), 
        { category_id:row.category_id, 'name':row.name, 'name_cn': row.name_cn})
      },
      { default: () => "update" }
    );
  }
  ...
  render(row) {
    return h(
      'div',
      {
        ...
        onClick: () => Inertia.delete(route('admin.categories.destroy', row.id), {preserveState:false})
      },
      { default: () => "delete" }
    );
  }
